...
struct WGPUAdapterInfoRef : public WGPUAdapterInfo {
    ...
    #ifdef WEBGPU_RAII_DEBUG
        std::cout << "Tracking a WGPUAdapterInfo: " << reinterpret_cast<std::uintptr_t>( this ) << '\n';
    #endif
        }
        
        // No copying.
        WGPUAdapterInfoRef( const WGPUAdapterInfoRef& ) = delete;
        WGPUAdapterInfoRef& operator=( const WGPUAdapterInfoRef& ) = delete;
        
        // A destructor to free the members.
        ~WGPUAdapterInfoRef() {
    #ifdef WEBGPU_RAII_DEBUG
            std::cout << "wgpuAdapterInfoFreeMembers(): " << reinterpret_cast<std::uintptr_t>( this ) << '\n';
    #ifndef WEBGPU_RAII_LEAK
            wgpuAdapterInfoFreeMembers( *this );
            // Zero the fields after freeing.
            *static_cast<WGPUAdapterInfo*>(this) = WGPUAdapterInfo{};
    #endif
    #else
            wgpuAdapterInfoFreeMembers( *this );
    #endif
